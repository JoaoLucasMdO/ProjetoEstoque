...
export const Layout = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const productsWithImageURLs = response.data.map((product: Product) => {
          ...
          } else if (product.imagem && typeof product.imagem === 'object' && product.imagem.type === 'Buffer') {
  ...
  const handleDelete = async (id:number) => {
    ...
    var response
    try{
       response = api.delete(`produtos/${id}`, {headers: {
      ...
      if(response){
        window.alert((await response).data.mensagem);
      } else{
        ...
        <Grid item xs={12} sm={6} md={4} key={index}>
          <Card>
            ...
            <CardContent>
              ...
              <Button variant="outlined" style={{margin:15}}  onClick={() => handleOpenEditForm(product.id)}>Editar Produto</Button>
            </CardContent>
          </Card>
        </Grid>
      ))}
      <EditProductForm open={openEditForm} onClose={handleCloseEditForm} id={id} />
    </Grid>
  );
};
